...
async function start() {
  const client = new _mongodb.MongoClient(`mongodb+srv://tpawelec:YWvPFFwfpGgDV3j5@cluster0.hvb1dn1.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0`);
  ...
  async function populateCartIds(ids) {
    await client.connect();
    const db = client.db('vue_course');
    return Promise.all(ids.map(id => db.collection('products').findOne({
      id
    })));
  }
